page 70174 "Loan Request Ess"
{
    Caption = 'Loan Request ESS';
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                {
                    // Start 26.06.2020
                    // @mma0786 Commented due to New requirement by Venkat
                    //Editable = VarEditable;
                    Editable = false;
                    // Stop 25.06.2020
                    ApplicationArea = All;
                ...
                {
                    Editable = ReopenBoolG;//Rec."WorkFlow Status" = Rec."WorkFlow Status"::Released;
                    ApplicationArea = All;
                }
        ...
        area(processing)
        {
            group(Approval)
            {
                ...
                action("Create Journal")
                {
                    ...
                    trigger OnAction()
                    begin
                        if (Rec."Posting Type" = Rec."Posting Type"::Finance)
                            and (Rec."Journal Created" = false) then
                            CreateJournal;


                        if (Rec."Posting Type" = Rec."Posting Type"::Payroll)
                            and (Rec."Journal Created" = false) then
                            PayrollAdjmtJournal;
                    end;
                }
    ...
    begin
        ...
        //PayPeriod.SetRange("Period Start Date", Rec."Repayment Start Date");
    ...
    begin
        ...
        if (NOT OpenApprovalEntriesExist) AND (Rec."Workflow Status" = Rec."Workflow Status"::Open) then
            SendForApprovalBoolG := true
        else
            SendForApprovalBoolG := false;

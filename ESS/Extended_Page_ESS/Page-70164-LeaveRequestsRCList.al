page 70164 "Leave Requests RC List"
{
    Caption = 'Leave Request List ESS';
    CardPageID = "Leave Request Card ESS";
    PageType = List;
        ...
        area(Creation)
        {
            action(New)
            {
                ...
                var
                    ...
                    Rec2: Record "Leave Request Header";
                BEGIN
                    // Start 28.09.2020
                    Rec2.Reset();
                    Rec2.Init();
                    if Rec2."Leave Request ID" = '' then begin
                        Rec2.Validate("Personnel Number", EssCUL.GetEmployeeID(Database.UserId));
                        Rec2.Validate(User_ID, Database.UserId);
                        Rec2.Insert(true);
                        Page.Run(page::"Leave Request Card ESS", Rec2);
                    end;
                    // Stop 28.09.2020
                END;
            }
    ...
    begin
        Rec.FilterGroup(2);
        Rec.SetRange("Personnel Number", Rcodeunit.GetEmployeeID(UserId));
        Rec.SetRange(User_ID, Database.UserId);
        Rec.FilterGroup(0);
    end;
    ...
    begin
        ...
        if DutyResume.FINDFIRST then begin
            DutyResume.FILTERGROUP(2);
            P_DutyResume.SETTABLEVIEW(DutyResume);
            P_DutyResume.RUN;
            DutyResume.FILTERGROUP(0);
        end;
    end;
}
